...
const TAXONOMY = [
  ...
  },
  {
    ...
    markers: [
      {
        label: "Self-directed harm or suicidal ideation (disclosed)",
        weight: 1.7,
        source: "parent",
        specificity: "very high",
        ageRange: { min: 24, max: 999, unit: "months" },
        ...
        regex: [
          "want(s|ed)?\\s+to\\s+(die|not\\s+be\\s+(alive|here))",
          "(doesn'?t|does\\s+not|don'?t|did\\s+not|didn'?t)\\s+want\\s+to\\s+(live|be\\s+(alive|here|around))",
          "not\\s+want(ing)?\\s+to\\s+be\\s+(alive|here|around)",
          "wish(es|ed)?\\s+(she|he|they|i)\\s+(was|were|wasn'?t|weren'?t)\\s+(never\\s+born|not\\s+here|here|dead|gone)",
          "(talk(s|ed|ing)?|thinking)\\s+about\\s+not\\s+(being\\s+(here|around|alive)|existing)",
          "(go|going)\\s+to\\s+sleep\\s+and\\s+not\\s+wak(e|ing)\\s+up",
          "want(s|ed)?\\s+to\\s+be\\s+(gone|dead)",
          "kill\\s+(her|him|them|my)self",
          "end\\s+(her|his|their|my)\\s+(life|it\\s+all)",
          "hit(s|ting)?\\s+(her|him|them|it)self",
          "hit(s|ting)?\\s+(her|his|their)\\s+(own\\s+)?(arms?|head|body)",
          "punch(es|ing)?\\s+(her|him|them)self",
          "bang(s|ing)?\\s+(her|his|their)\\s+head",
          "head[-\\s]?bang",
          "bit(es|ing)?\\s+(her|him|them)self",
          "scratch(es|ing)?\\s+(at\\s+)?(her|his|their)\\s+(own\\s+)?(skin|arms?|face)",
          "pinch(es|ing)?\\s+(her|him|them)self",
          "pick(s|ing)?\\s+(at\\s+)?(her|his|their)\\s+(own\\s+)?(skin|arms?|scabs?)",
          "cut(s|ting)?\\s+(her|him|them)self",
          "scratch(es|ing)?\\s+(her|him|them)self",
          "self[-\\s]?harm",
          "self[-\\s]?injur",
          "self[-\\s]?destruct",
          "hurt(s|ing)?\\s+(her|him|them)self",
          "suicid",
          "passive\\s+(SI|ideation)",
          "wants?\\s+to\\s+disappear",
          "better\\s+off\\s+(dead|without|if)",
          "life\\s+(isn'?t|is\\s+not|ain'?t)\\s+worth",
          "(there'?s|there\\s+is)\\s+no\\s+point",
          "hat(es|ed|ing)?\\s+(her|him|them)self",
        ],
        negationRequired: false,
        crossTags: ["RISK", "SELF_DIRECTED"],
        auContext: false,
      },
      {
